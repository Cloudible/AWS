...
export const saveCredentials = (
  ...
  try {
    ...
    console.log(
      `자격 증명이 암호화되어 파일에 저장되었습니다. (사용자: ${userId})`
    );
...
export const loadCredentials = (
  ...
  try {
    ...
    // 암호화된 데이터 복호화
    const decryptedData = decryptCredentials(
    ...
    console.log(
      `파일에서 자격 증명을 복호화하여 불러왔습니다. (사용자: ${userId})`
    );
  ...
  try {
    ...
    if (!checkCredentials(userId)) {
      return {
        valid: false,
        error:
          "자격 증명이 설정되지 않았습니다. /aws configure 또는 /aws load-credentials 명령어로 자격 증명을 설정하세요.",
      };
    }
...
export const getIAMUserInfo = async (
  userId: string,
  username: string
) => {
  try {
    ...
    const response = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () =>
            reject(new Error("IAM API 호출 시간 초과 (5초)")),
          5000
        )
      ),
    ]);
...
export const generateAWSConsoleUrl = (
