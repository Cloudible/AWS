...
const isValidDBIdentifier = (identifier: string): boolean => {
  ...
  if (identifier.includes("--")) return false; // 연속된 하이픈 불가
  ...
  execute: async (client, interaction) => {
    ...
    try {
      if (sub === "list") {
        ...
        const engine =
          interaction.options.getString("engine") || "mysql";
        const instanceClass =
          interaction.options.getString("instance-class") ||
          "db.m7i.large"; // 🔄 SQL Server 호환 인스턴스로 기본값 변경
        const storage =
          interaction.options.getInteger("storage") || 20;
        ...
        try {
          // 백그라운드에서 RDS 인스턴스 생성
          await createRDSInstance(userId, {
